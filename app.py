...
from db import (
    ...
    insert_songs_bulk,
    login_user,
    mark_song_pending_download,
    mark_song_played,
    reject_and_delete_song,
    update_bar_identity,
)
...
st.set_page_config(
    page_title="MelodicSaaS | Software para Bares",
    page_icon="🤖",
    layout="centered",
...
if mesa_param and url_bar_id:
    ...
    with col1:
        if logo_url:
            st.markdown(
                f"<img src='{logo_url}' style='max-height: 60px; margin-bottom: 10px;' />",
    ...
    if not q_df.empty:
        st.markdown(
            "<p style='font-size: 0.85em; color: #a0a0a0; margin-bottom: 5px; font-weight: bold;'>⏳ Próximas Peticiones:</p>",
            unsafe_allow_html=True,
        )
        tags_html = ""
    ...
    if active_requests >= 3:
        st.error(
            "⏳ Límite de 3 pedidos activos alcanzado. Espera tu turno mágico para pedir más."
        )
    else:
        # =================================================
        ...
        st.markdown(
            "<div class='magic-text' style='margin-top:15px;'>✨ LA MAGIA ESTÁ AQUÍ 👇</div>",
            unsafe_allow_html=True,
        )
        ...
        with col_m2:
            ...
            if speech_to_text:
                text_from_voice = speech_to_text(
                    language="es-ES",
                    start_prompt="🎙️ TÓCAME Y CANTA UN PEDACITO",
                    stop_prompt="⏳ TE ESTOY ESCUCHANDO...",
                    just_once=True,
                    key=st.session_state.stt_key,
                )
                st.markdown(
                    "<p style='font-size:0.7em; color:#888; text-align:center; margin-top:-5px;'><i>Filtro Inteligente: Acerca el celular</i></p>",
                    unsafe_allow_html=True,
                )
            ...
            if st.session_state.voice_memory:
                st.markdown(
                    f"<div style='text-align:center; color:{secondary_c}; font-weight:bold; margin-bottom:10px;'>🎤 Escuché: \"{st.session_state.voice_memory}\"</div>",
                    unsafe_allow_html=True,
                )
        ...
        st.markdown(
            "<p style='text-align:center; color:#A0A0A0; font-size:0.85em; margin-bottom: 2px;'>o usa el catálogo tradicional:</p>",
            unsafe_allow_html=True,
        )
        ...
        with col_artist:
            artistas = ["Explorar Artista..."] + sorted(
                songs_df["artist"].unique().tolist()
            )
            selected_artist = st.selectbox(
                "🎤 Filtrar Artista:",
                artistas,
                key="artist_filter",
                on_change=clear_voice,
        ...
        if search:
            ...
            is_semantic_enabled = (
                bool(st.session_state.voice_memory) or len(search) > 15
            )
            if is_semantic_enabled and "lyrics" in songs_df.columns:
                valid_lyrics_df = songs_df.dropna(subset=["lyrics"])
                if artist_chosen:
                    valid_lyrics_df = valid_lyrics_df[
                        valid_lyrics_df["artist"] == selected_artist
                    ]
            ...
            filtered_df = (
                pd.concat([exact_matches, semantic_matches])
            ...
            for _, row in filtered_df.iterrows():
                ...
                if (
                    ...
                    if search_lower in lyrics_lower:
                        ...
                        idx = -1
                        for w in [
                            word for word in search_lower.split() if len(word) >= 4
                        ]:
                    ...
                    if idx != -1:
                        start = max(0, idx - 40)
                            ...
                            len(row["lyrics"]),
                            idx + len(st.session_state.voice_memory) + 40,
                        )
                        ...
                        for w in [
                            ...
                            texto_cortado = re.sub(
                                f"(?i)({re.escape(w)})",
                                f"<b style='color:#FFF; background-color:{secondary_c}; padding:0 3px; border-radius:3px;'>\\1</b>",
                                texto_cortado,
                            )
                ...
                with cols[1]:
                    ...
                    if st.button(
                        ...
                        on_click=handle_pedir,
                        args=(url_bar_id, row["id"], mesa_id),
                    ):
                        st.balloons()
    ...
    if not alerts_df.empty:
        for _, alert in alerts_df.iterrows():
            if alert["status"] == "pending_download":
                st.warning(
                    f"📻 El DJ está buscando/descargando **{alert['title']}** ({alert['artist']}) para ti. ¡No gasta tu cupo!"
                )
            elif alert["status"] == "rejected":
                st.error(
                    f"❌ **{alert['title']}** ({alert['artist']}) fue eliminada por el DJ (Música fuera de contexto). Se te devolvió tu cupo."
                )
...
# =======================================================
    ...
    if "admin_logged_in" not in st.session_state:
        ...
        st.markdown(
            "<h1 style='text-align:center;'>MelodicSaaS | Portal Propietarios</h1>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<p style='text-align:center; color:#A0A0A0;'>Inicia Sesión en el Universo Privado de tu Bar</p><br>",
            unsafe_allow_html=True,
        )
    ...
    # SI EL DUEÑO ESTÁ LOGEADO, MUESTRA SU PANEL (VISTA DJ)
        ...
        with col1:
            if logo_url:
                st.markdown(
                    f"<img src='{logo_url}' style='max-height: 80px; margin-bottom: 10px;' />",
                    unsafe_allow_html=True,
                )
            else:
        ...
        st.markdown(
            "<p style='color:#A0A0A0; font-size: 0.9em; margin-top:-10px;'>Las 4 canciones que más facturan en tu bar.</p>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            "<p style='color:#A0A0A0; font-size: 0.9em; margin-top:-10px;'>Las apartaste porque no las tenías en tu PC local/Spotify.</p>",
            unsafe_allow_html=True,
        )
        ...
        if backlog_df.empty:
            ...
            for idx, row in backlog_df.iterrows():
                with st.container():
                    ...
                    if st.button(
                        f"✅ Ya la descargué (Marcar Sonada)",
                        key=f"res_{row['song_id']}",
                        use_container_width=True,
                    ):
                        mark_song_played(bar_id, row["song_id"])
                        st.rerun()
        ...
        st.success(
            f"Estás dentro de tu propio universo de datos. Tus pedidos son 100% privados a tu negocio."
        )
        st.info(
            f"👉 **Enlace para tus clientes (Imprime el QR):** `URL/?bar={bar_id}&mesa=X`"
        )
        ...
        with st.expander("⚙️ Ajustes de Identidad Visual (Tu Marca)"):
            st.markdown(
                "<p style='color:#bbb;'>Personaliza los colores y el Logo de tu Bar.</p>",
                unsafe_allow_html=True,
            )
            ...
            with col_cc2:
                new_secondary = st.color_picker(
                    "Color Secundario (Brillo/Neón)", secondary_c
                )

            uploaded_logo = st.file_uploader(
                "Sube o arrastra el Logo de tu Bar", type=["png", "jpg", "jpeg", "webp"]
            )
