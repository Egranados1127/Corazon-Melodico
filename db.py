...
def add_request(bar_id, song_id, table_id):
    try:
        ...
        supabase.table("requests_saas").insert(
            {"bar_id": bar_id, "song_id": int(song_id), "table_id": table_id}
        ).execute()
...
def get_top_songs(bar_id, limit=4):
    res = (
        supabase.table("requests_saas")
        .select("*, songs_saas(title, artist)")
        .eq("bar_id", bar_id)
        .execute()
    )
    data = res.data
...
def get_queue(bar_id):
    ...
    grouped = (
        grouped.groupby(["song_id", "title", "artist"])
        .agg(
    ...
    grouped["oldest_request_time"] = pd.to_datetime(
        grouped["oldest_request_time"], errors="coerce", utc=True
    ).dt.tz_localize(None)
    grouped["minutes_waiting"] = (
        now - grouped["oldest_request_time"]
    ).dt.total_seconds() / 60.0
    grouped["minutes_waiting"] = grouped["minutes_waiting"].clip(lower=0)
    grouped["score"] = (grouped["total_requests"] * 10) + (
        np.log1p(grouped["minutes_waiting"]) * 5
    )
...
def mark_song_played(bar_id, song_id):
    ...
    supabase.table("requests_saas").update({"status": "played"}).eq(
        "bar_id", bar_id
    ).eq("song_id", song_id).in_("status", ["pending", "pending_download"]).execute()


def mark_song_pending_download(bar_id, song_id):
    supabase.table("requests_saas").update({"status": "pending_download"}).eq(
        "bar_id", bar_id
    ).eq("song_id", song_id).eq("status", "pending").execute()


def reject_and_delete_song(bar_id, song_id):
    try:
        supabase.table("requests_saas").update({"status": "rejected"}).eq(
            "bar_id", bar_id
        ).eq("song_id", song_id).eq("status", "pending").execute()
        supabase.table("songs_saas").update({"is_banned": 1}).eq("bar_id", bar_id).eq(
            "id", song_id
        ).execute()
    except:
        pass
...
def get_table_alerts(bar_id, table_id):
    res = (
        ...
        .eq("bar_id", bar_id)
        .eq("table_id", table_id)
        .in_("status", ["pending_download", "rejected"])
        .execute()
    )
    data = res.data
    if not data:
    ...
    for r in data:
        ...
        if s:
            records.append(
                {"title": s["title"], "artist": s["artist"], "status": r["status"]}
            )
...
def get_download_backlog(bar_id):
    ...
    grouped = (
        df.groupby(["song_id", "title", "artist"])
        .agg(
            total_requests=("table_id", "count"),
...
def update_bar_identity(bar_id, primary, secondary, logo_url):
    supabase.table("bars").update(
        {"primary_color": primary, "secondary_color": secondary, "logo_url": logo_url}
